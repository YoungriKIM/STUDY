# 결측치를 없애는 방법들: 중간값, 평균값, 0으로, 위에 값, 아래 값, 행자체 삭제, 중위값(평균값과는 다르다.) 등
# 그리고! 보간법이 있다.
...
ts_intp_linear = ts.interpolate()   # interpolate : 말참견하다, 써넣음, 통계에서 사용, 보간법
...
# 쉽고 빠르지만 이 기능을 절대적으로 생각하지 말고 결측치를 처리하는 모든 방법을 사용해!