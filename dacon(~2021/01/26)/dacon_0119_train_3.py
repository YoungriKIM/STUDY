...
# print(dataset.shape)
dataset = dataset.iloc[:,[3,4,5,6,7,8]]
...
from tensorflow.keras.layers import Dense, LSTM, Conv1D, Flatten, MaxPooling1D, Dropout, Reshape, Conv2D, MaxPool2D

model = Sequential()
model.add(Conv2D(7, 2, input_shape=(x_train.shape[1], x_train.shape[2], x_train.shape[3]), padding='same', activation='relu'))
...
hist = model.fit(x_train, y_train, epochs=20, batch_size=14, validation_split=0.2, verbose=1, callbacks=[stop, reduce_lr])#,check])


#===================================================================
#4. 평가, 예측
result = model.evaluate(x_test, y_test, batch_size=14)
print('mse: ', result[0])
print('mae: ', result[1])
