...
model = Sequential()
model.add(Dense(5, input_dim=1, activation='linear')) #인풋이 시작하는 층임
model.add(Dense(3, activation='linear'))
model.add(Dense(4))
model.add(Dense(1))
#시퀀스라서 맨 윗줄만 인풋을 써주고 다른 애들은 안써줘도 된다.
...
result = model.predict([4]) #내가 예측하고 싶은 값을 넣는다. 이 경우에는 4를 넣었다. 이제 나오는 값은 x가 4일 때 y값이다.
print('result : ', result)
