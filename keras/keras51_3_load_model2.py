...
# 모델과 컴파일 훈련까지 저장한 h5 파일을 불러와서 적용하니 훈련을 안 돌고 바로 결과가 나온다. 
# 이 h5파일은 모델이 다 돌고나서 저장되는 걸 볼 수 있는데 이때 w까지 저장이 된 것이다.
# 이러면 w까지 저장된 모델을 불러와서 예측값만 바꾸면서 볼 수 있다. 원하는 방법으로 저장해서 쓰자.
