...
df = pd.read_csv('../data/csv/dacon1/train/train.csv')
...
y1=y[:,:,0].reshape(-1,96,) # 회귀 y
y2=y[:,:,1].reshape(-1,96,) # 이진 y
...
model2 = Sequential()
#model2.add(GRU(64,activation='relu',input_shape=(336,8)))
...
'''
modelpath = './데이콘/태양열/data/model_regressor_0119target_batch{}_epoch{}_validation_split{}.h5'.format(batch_size,epochs,validation_split)
...
model1 = Sequential()
model1.add(LSTM(32,activation='relu',input_shape=(336,8)))
...
model1.fit(x,[y1,y2],validation_split=validation_split,epochs=epochs,batch_size=batch_size,verbose=1,callbacks=[es,cp,reduce_lr])
'''
