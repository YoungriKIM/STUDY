...
# x, y 데이터 지정
# 액면분할 시점 : 1740
x = df0114.iloc[0:2399, [0,1,2,3,4,5,6,7]]
y = df0114.iloc[1:2400, 3]
x_pred = df0114.iloc[2399, [0,1,2,3,4,5,6,7]]
...
from tensorflow.keras.layers import Dense, Input, Dropout, Conv1D, Flatten, MaxPooling1D, LSTM, GRU, LeakyReLU, SimpleRNN

model = Sequential()
model.add(SimpleRNN(16, input_shape=(x_train.shape[1], x_train.shape[2]), activation='relu', return_sequences=False))
model.add(Dense(1))
...
hist = model.fit(x_train, y_train, epochs=1000, batch_size=8, validation_data=(x_val, y_val), verbose=1, callbacks=[stop, check])
...
# 16 mse:  807,768.1875
# 8, 200~   mse:  998,558.625
# 8, 0~ 6행 mse:  845,573.625
# 8, 0~ 7행 mse:  746,372.125
# 8, 0~ 8행 mse:  796,930.8125