...
# x, y 데이터 지정
# 액면분할 후
x = df2.iloc[2000:2399, [0,1,2,3,4,5,6]]
y = df2.iloc[2001:2400, 3]
x_pred = df2.iloc[2399, [0,1,2,3,4,5,6]]
...
model = Sequential()
model.add(Conv1D(filters = 800, kernel_size = 2, strides=1, padding = 'same', input_shape = (7,1), activation='relu'))
...
hist = model.fit(x_train, y_train, epochs=500, batch_size=8, validation_data=(x_val, y_val), verbose=1, callbacks=[stop]) #, check])
